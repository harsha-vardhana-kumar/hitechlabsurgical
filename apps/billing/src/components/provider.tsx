...
interface Context {
  ...
  session: Session;
  busy: boolean;
  run<T>(command: Command<T>, message?: string): Promise<T>;
  confirm(title: string, detail: string, action: () => Promise<unknown>): void;
  notify(message: string): void;
}
const WorkspaceContext = createContext<Context | null>(null);
export function useWorkspace() {
  const c = useContext(WorkspaceContext);
  if (!c) throw new Error("Workspace is not ready.");
  return c;
}
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  const [service] = useState(
    () => new BillingService(new DemoLocalAdapter(), new DemoAuthAdapter()),
  );
  ...
  const load = useCallback(() => {
    const result =
      environment.mode === "demo"
        ...
        : Promise.reject(
            new Error(
              "Production authentication and API adapters are not configured. Set NEXT_PUBLIC_BILLING_MODE=demo for the local demo.",
            ),
          );
    return result
      .then((r) => {
        setW((current) =>
          !current || r.workspace.revision >= current.revision
            ? r.workspace
            : current,
        );
  ...
  async function run<T>(command: Command<T>, message = "Saved successfully") {
    setBusy(true);
    try {
      const r = await service.execute(command);
      setW((current) =>
        !current || r.workspace.revision >= current.revision
          ? r.workspace
          : current,
  ...
  if (!w || !session)
    return (
      <main className="startup">
        ...
        <p>{error || "Preparing the demo workspace in this browser."}</p>
          ...
          <button
            ...
            onClick={async () => {
              try {
                ...
                /* Error is surfaced by run. Keep the dialog open for correction. */
              }
            }}
          >
            {busy ? "Saving…" : "Confirm"}
  ...
  useEffect(() => {
    ...
    const click = (e: MouseEvent) => {
      ...
      if (
        ...
        return;
      if (!window.confirm("Leave this form? Unsaved changes will be lost.")) {
        e.preventDefault();
        e.stopPropagation();
      }
    };
